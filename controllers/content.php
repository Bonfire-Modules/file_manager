...
//		Template::set('datatableData', $datatableData);
		...
		$records = $this->file_manager_files_model->select('id, id as thumbnail, file_name, description, tags, public, extension')->find_all();
		...
		if($records)
		{
			...
			{
				$records[$key]->thumbnail = '<img src="' . site_url(SITE_AREA .'/content/file_manager/view_image/thumbnail/' . $record->id) . '" />';
				$records[$key]->public = $records[$key]->public ? lang('file_manager_yes') : lang('file_manager_no');
				if($this->icon_exists($records[$key]->extension) !== false) {
					$records[$key]->extension = '<img src="' . site_url(SITE_AREA .'/content/file_manager/icon/' . $record->extension) . '.png" />';
				}
				$records[$key]->sha1_checksum = '<a target="_blank" class="btn btn-mini" href="' . site_url(SITE_AREA .'/widget/file_manager/download/' . $record->id) . '"><i class="icon-download-alt">&nbsp;</i> Download</a>';
