			...
			<select id="alias_target_model" name="alias_target_model">
				...
				<?php foreach($module_models as $module_name => $module) : ?>
					<?php foreach($module['models'] as $model) : ?>
						<option value='<?php echo $model; ?>' class='<?php echo $module_name; ?>'<?php if(isset($_POST['alias_target_model']) && $_POST['alias_target_model'] == $model) echo ' selected'; ?>><?php echo ucfirst(preg_replace('/_/', ' ', $model)); ?></option>
					<?php endforeach; ?>
				<?php endforeach; ?>
			</select>
